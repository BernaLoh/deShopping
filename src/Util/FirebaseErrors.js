 ...
 * @param {string} errorMsj English error message
 * @return {string} 
 *         - If the code exists: It returns the translated error message text.
 *         - If the code doesn´t exist: It returns the errorMsj param without 'Firebase:' and '(auth/...)' code
 */
 export function FirebaseErrorsTranslations(errorCode, errorMsj) {
    const translations = {
    ...
    if (translations[errorCode]) {
      ...
      // Remove 'Firebase:' and '(auth/...)' code
      // Came errorMsj= Firebase: The password is invalid or the user does not have a password. (auth/wrong-password).
      // Returned errorMsj= The password is invalid or the user does not have a password.
      errorMsj = errorMsj.replace('Firebase: ','')
      errorMsj = errorMsj.replace(/\(auth\/.*\)\./,'')
    }
    return errorMsj
  }